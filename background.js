...
chrome.runtime.onConnect.addListener(function (port) {
    ...
    if(port.name === "MJcontent-script"){
        MJport = port;
    }
    port.onMessage.addListener(async function (msg) {
        console.log("Message from content:", { msg });
        // MJport.postMessage({ reply: "userID" });
        
        if (msg.msg == 'getThisUserJobsData') {
            port.postMessage({ reply: "ok" });
            MJport.postMessage({ msg: "thisUserJobsData"});
            let jobData = null;
            const handleRes = (e) => {
                // console.log({e});
                if(e.hasOwnProperty('thisUserJobsData')){
                    console.log('thisUserJobsData', e);
                    port.postMessage({ reply: "thisUserJobsData", thisUserJobsData: e.thisUserJobsData });
                    MJport.onMessage.removeListener(handleRes);
                }
            }
            MJport.onMessage.addListener(handleRes);
            while(jobData === null){
                await waitSeconds(1);
            }
        }
...
//     method: "POST",
//     headers: { "Content-Type": "application/json" },
//     body: JSON.stringify({ jobIds: ["4fd7a4f9-b447-46fe-aa41-f3990ff667ee"] }),
//     timeout: 10000
// }).then((response) => {
//     jobStatusResponse = response;
// }).catch((error) => {
